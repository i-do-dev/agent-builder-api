...
from api.db.migrations.env import Migration
...
def test_run_migrations_offline_calls_context(monkeypatch):
    ...
    monkeypatch.setattr("api.db.migrations.env.context", context_mock)
...
async def test_run_migrations_online(monkeypatch):
    ...
    monkeypatch.setattr("api.db.migrations.env.async_engine_from_config", MagicMock(return_value=connectable_mock))
    # Patch the do_run_migrations method to be a mock
    do_run_migrations_mock = MagicMock()
    monkeypatch.setattr("api.db.migrations.env.Migration.do_run_migrations", do_run_migrations_mock)
...
def test_migration_run(monkeypatch):
    ...
    monkeypatch.setattr("api.db.migrations.env.load_dotenv", MagicMock())
    # Patch asyncio.run to be a mock
    monkeypatch.setattr("api.db.migrations.env.asyncio.run", MagicMock())
    ...
    monkeypatch.setattr("api.db.migrations.env.Migration.__init__", mock_init)
    ...
    from api.db.migrations.env import load_dotenv, asyncio
